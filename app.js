...
class ChordAnnotatorApp {
    ...
    saveSongMeta() {
        ...
        this.currentSong.videoEndStillSeconds = this.getVideoEndStillSeconds();
    ...
    unlockAudioContext() {
        ...
        try {
            if (!this._htmlAudioUnlock) {
                this._htmlAudioUnlock = new Audio('data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA');
                this._htmlAudioUnlock.setAttribute('playsinline', '');
                this._htmlAudioUnlock.setAttribute('webkit-playsinline', '');
                this._htmlAudioUnlock.volume = 0.01;
            }
            const played = this._htmlAudioUnlock.play();
            if (played) played.catch(() => {});
        } catch {
            // keep Web Audio unlock below
        }
        try {
            ...
            // iOS may still unlock on resume()
        }
        return ctx;
    }
    ...
    playPianoChord(chord) {
        ...
        this.ensureGrandPianoSamples(ctx, notes);
    }
    ...
    setExportButtonsBusy(busy, videoLabel) {
        ...
        const videoEndStill = document.getElementById('videoEndStill');
        if (imageBtn) {
        ...
        if (videoEndStill) videoEndStill.disabled = busy;
        if (videoBtn) {
    ...
    lyricVideoSpec() {
        ...
        const holdEndMs = Math.min(
            this.getVideoEndStillSeconds() * 1000,
            Math.max(0, durationMs - holdStartMs)
        );
        return {
            width: 1080,
            height: 1920,
    ...
    paintLyricsCard(ctx, card, origin, clip) {
        ...
        card.querySelectorAll('.chord-label').forEach((el) => {
            ...
            ctx.textBaseline = 'middle';
            if (onText) {
                const rtl = card.classList.contains('rtl') || card.classList.contains('align-right');
                const pad = parseFloat(style.paddingInlineStart) || parseFloat(style.paddingLeft) || 5;
                ctx.textAlign = rtl ? 'right' : 'left';
                ctx.fillText(el.textContent || '', rtl ? box.x + box.w - pad : box.x + pad, box.y + box.h / 2);
            } else {
                ctx.textAlign = 'left';
                const pad = parseFloat(style.paddingInlineStart) || parseFloat(style.paddingLeft) || 5;
    ...
    chordColorKey(chord) {
        return this.displayChord(chord || '').trim().toLowerCase();
    }

    bindChordColorPicker() {
        const button = document.getElementById('chordColorBtn');
        ...
        this.colorPalette.forEach((color) => {
            ...
            swatch.type = 'button';
            swatch.className = 'chord-color-swatch';
            swatch.style.backgroundColor = color;
            swatch.dataset.color = color;
            swatch.setAttribute('aria-label', `Use color ${color}`);
            swatch.addEventListener('click', () => this.pickSongChordColor(color));
            palette.appendChild(swatch);
        });
        button.addEventListener('click', (e) => {
            e.stopPropagation();
            const open = palette.hidden;
            this.setChordColorPaletteOpen(open);
        });
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.chord-color-wrap')) this.setChordColorPaletteOpen(false);
        });
    }

    setChordColorPaletteOpen(open) {
        const button = document.getElementById('chordColorBtn');
        const palette = document.getElementById('chordColorPalette');
        if (!button || !palette) return;
        palette.hidden = !open;
        button.setAttribute('aria-expanded', open ? 'true' : 'false');
        if (open) this.syncChordColorSwatches();
    }

    updateChordColorButton() {
        const button = document.getElementById('chordColorBtn');
        if (!button) return;
        const chord = this.displayChord(document.getElementById('chordInput')?.value || '').trim();
        button.hidden = !chord;
    ...
    getChordColor(chord) {
        ...
        const match = Object.keys(map).find((name) => this.chordColorKey(name) === key);
    ...
    countUniqueChords(song) {
        ...
        (song?.annotations || []).forEach((annotation) => {
            const name = this.displayChord(annotation.chord || '').trim().toLowerCase();
    ...
    renderSongList() {
        ...
        this.songs.forEach((song, index) => {
            ...
            const chordCount = this.countUniqueChords(song);
